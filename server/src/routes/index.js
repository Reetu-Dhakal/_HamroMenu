...
router.get('/subscription-plans', asyncHandler(async (_req, res) => {
  const plans = await SubscriptionPlan.find({ isActive: true }).sort({ price: 1 });
  return ApiResponse.send(res, 200, plans, 'Plans retrieved');
}));

// ─── Restaurant owner subscription & billing ──────────────────────────
router.get('/my/subscription', auth, authorize(USER_ROLES.ADMIN), asyncHandler(async (req, res) => {
  const subscription = await Subscription.findOne({ restaurant: req.user.restaurant })
    .populate('plan');
  if (!subscription) return ApiResponse.send(res, 200, null, 'No subscription');
  return ApiResponse.send(res, 200, subscription, 'Subscription retrieved');
}));

router.get('/my/usage', auth, authorize(USER_ROLES.ADMIN), asyncHandler(async (req, res) => {
  const subscription = await Subscription.findOne({ restaurant: req.user.restaurant }).populate('plan');
  if (!subscription || !subscription.plan) return ApiResponse.send(res, 200, { tables: 0, menuItems: 0, staff: 0 }, 'No subscription');
  const Menu = (await import('../models/MenuItem.js')).default;
  const Table = (await import('../models/Table.js')).default;
  const Staff = (await import('../models/Staff.js')).default;
  const KitchenStaff = (await import('../models/KitchenStaff.js')).default;
  const [tableCount, menuItemCount, staffCount, kitchenCount] = await Promise.all([
    Table.countDocuments({ restaurant: req.user.restaurant, isActive: true }),
    Menu.countDocuments({ restaurant: req.user.restaurant, isAvailable: true }),
    Staff.countDocuments({ restaurant: req.user.restaurant }),
    KitchenStaff.countDocuments({ restaurant: req.user.restaurant }),
  ]);
  return ApiResponse.send(res, 200, {
    tables: { used: tableCount, max: subscription.plan.maxTables },
    menuItems: { used: menuItemCount, max: subscription.plan.maxMenuItems },
    staff: { used: staffCount + kitchenCount, max: subscription.plan.maxStaffAccounts },
  }, 'Usage retrieved');
}));

router.get('/my/invoices', auth, authorize(USER_ROLES.ADMIN), asyncHandler(async (req, res) => {
  const invoices = await Invoice.find({ restaurant: req.user.restaurant }).sort({ createdAt: -1 }).limit(24);
  return ApiResponse.send(res, 200, invoices, 'Invoices retrieved');
}));

router.post('/my/subscription/change-plan', auth, authorize(USER_ROLES.ADMIN), asyncHandler(async (req, res) => {
  ...
  subscription.status = plan.price === 0 ? 'TRIALING' : 'ACTIVE';
  subscription.currentPeriodStart = new Date();
  const periodEnd = new Date();
  periodEnd.setMonth(periodEnd.getMonth() + 1);
  subscription.currentPeriodEnd = periodEnd;
  await subscription.save();
  return ApiResponse.send(res, 200, subscription, 'Plan changed');
}));
...
router.get('/super-admin/subscriptions', auth, authorize(USER_ROLES.SUPER_ADMIN), superAdminController.subscriptions);
router.get('/super-admin/revenue', auth, authorize(USER_ROLES.SUPER_ADMIN), superAdminController.revenueOverview);
router.get('/super-admin/restaurants/:restaurantId/reports', auth, authorize(USER_ROLES.SUPER_ADMIN), superAdminController.restaurantReports);
router.get('/super-admin/restaurants/:restaurantId/subscription', auth, authorize(USER_ROLES.SUPER_ADMIN), superAdminController.subscriptionsByPlan);

// Super admin user management
router.get('/super-admin/users', auth, authorize(USER_ROLES.SUPER_ADMIN), asyncHandler(async (req, res) => {
  const { role, page = 1, limit = 20 } = req.query;
  const CustomerModel = (await import('../models/Customer.js')).default;
  const StaffModel = (await import('../models/Staff.js')).default;
  const KitchenStaffModel = (await import('../models/KitchenStaff.js')).default;
  const AdminModel = (await import('../models/Admin.js')).default;
  const SuperAdminModel = (await import('../models/SuperAdmin.js')).default;
  const models = { customer: CustomerModel, staff: StaffModel, kitchen: KitchenStaffModel, admin: AdminModel, super_admin: SuperAdminModel };
  if (role && models[role]) {
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const [users, total] = await Promise.all([
      models[role].find().select('-password -refreshToken -resetPasswordToken -resetPasswordExpires').skip(skip).limit(parseInt(limit)).sort({ createdAt: -1 }),
      models[role].countDocuments(),
    ]);
    return ApiResponse.send(res, 200, { users, pagination: { page: parseInt(page), limit: parseInt(limit), total, totalPages: Math.ceil(total / parseInt(limit)) } }, `${role} users retrieved`);
  }
  const [customers, staff, kitchen, admins, superAdmins] = await Promise.all([
    CustomerModel.countDocuments(),
    StaffModel.countDocuments(),
    KitchenStaffModel.countDocuments(),
    AdminModel.countDocuments(),
    SuperAdminModel.countDocuments(),
  ]);
  return ApiResponse.send(res, 200, { counts: { customers, staff, kitchen, admins, superAdmins } }, 'User counts retrieved');
}));

router.get('/super-admin/plans', auth, authorize(USER_ROLES.SUPER_ADMIN), asyncHandler(async (_req, res) => {
  const plans = await SubscriptionPlan.find().sort({ price: 1 });
  return ApiResponse.send(res, 200, plans, 'Plans retrieved');
}));

router.post('/super-admin/plans', auth, authorize(USER_ROLES.SUPER_ADMIN), asyncHandler(async (req, res) => {
  const plan = await SubscriptionPlan.create(req.body);
  return ApiResponse.send(res, 201, plan, 'Plan created');
}));

router.put('/super-admin/plans/:id', auth, authorize(USER_ROLES.SUPER_ADMIN), asyncHandler(async (req, res) => {
  const plan = await SubscriptionPlan.findByIdAndUpdate(req.params.id, req.body, { new: true });
  if (!plan) throw new ApiError(404, 'Plan not found');
  return ApiResponse.send(res, 200, plan, 'Plan updated');
}));
...
router.patch('/admin/:restaurantId/profile', auth, authorize(USER_ROLES.ADMIN), ensureRestaurantContext, asyncHandler(async (req, res) => {
  const allowed = ['name', 'description', 'tagline', 'cuisine', 'address', 'contact', 'operatingHours', 'logoUrl', 'coverUrl'];
  const updates = {};
  for (const key of allowed) {
    if (req.body[key] !== undefined) updates[key] = req.body[key];
  }
  const restaurant = await Restaurant.findByIdAndUpdate(req.params.restaurantId, updates, { new: true, runValidators: true });
  if (!restaurant) throw new ApiError(404, 'Restaurant not found');
  return ApiResponse.send(res, 200, restaurant, 'Restaurant profile updated');
}));
